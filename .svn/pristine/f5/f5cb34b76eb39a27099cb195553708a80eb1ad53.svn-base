...
function goListPage(bSaved, breadCrumbs) {
	...
	if (breadCrumbs != null && breadCrumbs != "") {
		...
		var keyName = arr[1];
		var keyValue = arr[2];
		movePageCommon(url, keyName, keyValue, "breadCrumbs", breadCrumbs);
	} else {
		movePageCommon("list");
	}
}
...
function movePageCommon(url, keyName, keyValue) {
	// history.pushState({}, "", "");
	$("#commonForm").remove();
...
window.onpopstate = function(event) {
	...
	case 0:
		...
		if (breadCrumbs.length > 0) {
			movePageCommon(document.referrer, "breadCrumbs", breadCrumbs.val());
		} else {
			movePageCommon(document.referrer);
		}*/
		break;
	case 1:
...
function showGlanceViewContextMenu(node) {
	...
	if (node.data.type == "UseCase") {
		updateUrl = "javascript:movePageCommon('../usecase/update', 'name', '"+node.data.title+"')";
		copyUrl = "javascript:movePageCommon('../usecase/copy', 'name', '"+node.data.title+"')";
	} else if (node.data.type == "RuleSet") {
		updateUrl = "javascript:movePageCommon('../ruleset/update', 'ruleSetName', '"+node.data.title+"')";
		copyUrl = "javascript:movePageCommon('../ruleset/copy', 'ruleSetName', '"+node.data.title+"')";
	} else if (node.data.type == "Rule") {
		updateUrl = "javascript:movePageCommon('../rule/update', 'name', '"+node.data.title+"')";
		copyUrl = "javascript:movePageCommon('../rule/copy', 'name', '"+node.data.title+"')";
	} else {
		return;
	}
