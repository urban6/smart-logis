...
(function(global) {
  ...
  extend(fabric, {
    ...
    getGradientDefs: function(doc) {
      var linearGradie = doc.getElementsByTagName('linearGradient'),
          radialGradie = doc.getElementsByTagName('radialGradient'),
          el, i, j = 0, id, xlink, elList = [ ],
          gradientDefs = { }, idsToXlinkMap = { };

      elList.length = linearGradie.length + radialGradie.length;
      i = linearGradie.length;
      while (i--) {
        elList[j++] = linearGradie[i];
      }
      i = radialGradie.length;
      while (i--) {
        elList[j++] = radialGradie[i];
      }
