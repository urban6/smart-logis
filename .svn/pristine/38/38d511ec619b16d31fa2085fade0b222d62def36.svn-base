...
<script type="text/javascript" charset="utf-8" src="/scripts/fileupload.js"></script>
	<input type="hidden" id="navi2_nm" value="관리자 수정" />
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#btn_attach").click(function(){
	 	...
	 	$('#btnResetPwd').on('click', function(){
	 		openConfirmModal("NOTIFICATION","비밀번호를 초기화하시겠습니까?<br>등록된 휴대폰 번호로\r\n임시 비밀번호가 발송됩니다", function() {
				showLoading();
				$.ajax({
	                url : "resetPassword",
	                 type : 'POST',
	...
	function goCancel(){
		$("#myForm").attr("action", "/management/partnermng/partneradmin/list");
		$("#myForm").submit();
	}
	...
	function uploadSucc(result){
		...
		$('#btn_attach').attr("disabled", "disabled");
	}
	...
	function deleteImageAction(obj){
		...
		$("#img_file_sno").val(0);
		$('#btn_attach').removeAttr("disabled");
	}
</script>

<style type="text/css">
.__NEW .form_table tr td .btn_black {height: 30px;width: 120px;line-height: 28px;}
</style>

<section id="content" class="w1200">
	<article class="admin_conf">
	<form id="myForm" name="myForm" method="POST" class="valid_form">
		<input type="hidden" id=search_partner_clcd name="search_partner_clcd" value="${partnerAdminDomain.search_partner_clcd}">
		<input type="hidden" id=search_partner_sno name="search_partner_sno" value="${partnerAdminDomain.search_partner_sno}">
		<input type="hidden" id=search_type name="search_type" value="${partnerAdminDomain.search_type}">
		<input type="hidden" id="search_text" name="search_text" value="${partnerAdminDomain.search_text}">	
	          			...
	          			<tbody>
							...
							<tr>
								...
								<td>
									...
									<button type="button" class="btn_black" id="btnResetPwd">비밀번호 초기화</button>
									</shovvel:auth>
								</td>
				            </tr>		
							<tr>
