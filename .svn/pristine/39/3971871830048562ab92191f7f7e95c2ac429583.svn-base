...
/**
 * 모달 창을 닫는 펑션 selector에 해당 하는 모달창을 닫는다. selector를 입력하지 않으면 모든 모달창들과 overlay를
 * 닫는다.
 *
 * @param selector :
 *            닫으려는 모달창의 jquery selector
 */
...
/**
 * 이미 모달창에 존재하는 메시지인지 확인한다. 메시지가 현재 h4태그에 표시되는데 HTML구조가 변경될 수 있어 html 문자열을 모두
 * 가져와 검색합니다.
 *
 * @param modalMsg :
 *            모달 창에 띄울 메시지
...
/**
 * 컨테이너에 모달을 추가하는 펑션 기존 jquery.simple.js는 1개의 모달창만 만들 수 있다. 모달 창 중첩 기능 구현을 위하여
 * 해당 펑션을 제작함
 *
 * @param selector :
 *            새롭게 모달을 띄울 태그의 jquery selector
 */
...
/*
 * openConfirmModal("Message", "Description", function() {console.log("ok");},
 * function() {console.log("cancel")}, "OK", "Cancel");
 */
...
/*
 * openAlertModal("Message", "Description", function() {console.log("ok");},
 * "OK");
 */
...
function openAlertModal(sMessage, sDescription) {
	console.log("openAlertModal 2");
	fnErrorMessageTitle(sMessage, sDescription);
}
...
/**
 * 팝업 존재 여부에 따라, Scroll 활성/비활성 (팝업 O : 비활성, 팝업 X : 활성)
 */
function setScroll(){
  if($('.popup.on').length){
