...
<head>
    ...
    <script type="text/javascript">
        ...
        function setFirstDate() {
            ...
            document.getElementById('startDate').value = year + "-" + month + "-" + (startDate);
        	 ...
        	 $.ajax("/order/OrderList",
        			 
                     {
                         ...
                         dataType: 'JSON'
                          ...
                          if (list.length === 0) {
                              orderStr += '<div style="text-align: center; padding: 5%"><span class="span-empty">조회된 데이터가 없습니다.</span></div>';
                          }

                          $.each(list, function (i) {
                                                      	                              
                                  orderStr += '<li class="order">' +
                                      '<article>' +
                                      '<div class="description">' +                                      
                                      '<h1>' + list[i].productId + '</h1>' +
                                      '<loginid>' +
                                      '<span class="span-loginid">' + list[i].loginId + '</span>' +
                                      '</loginid>' +
                                      '</div>' +
                                      '<div class="orderdate">' +
                                      '<div><span class="span-orderdate">' + list[i].orderDate + '</span></div>' +
                                      '<div class="status">' + list[i].orderProductStatusName + '</div>' +
                                      '<input type="button" class="btn btn-logis-primary" style="width: 60%" value="상세보기"/>' +
                                      '</div></article></li>';
                          ...
                          $("#list").find('input').click(function (data) {
                              ...
                              getOrderDetail(list[index].orderUid);
        ...
        function getOrderDetail(uid) {
            ...
            const orderUid = document.createElement('input');
            
            orderUid.setAttribute("name", "orderUid");
            orderUid.setAttribute("value", uid);
            
            const form = document.createElement('form');
            
            form.setAttribute('method', 'POST');
            form.setAttribute('action', '/order/OrderDetail');
                        
            form.appendChild(orderUid);
...
<div id="wrap">
    <div class="section">
        <div class="div-search-box">
            <!-- <form class="form-search-box" action="/order/OrderList" method="post"> -->             
                ...
                <div class="div-space-box">
                    <div class="form-group">
                        <div style="width: 120%">
                            ...
                            <input class="form-control" type="text" id="keyword" maxlength="20" onkeydown="nonPressKor(this);">
                        </div>
                    </div>
                </div>
                <div class="div-confirm-box">
                    <input id="btnSearch" type="submit" class="btn btn-logis-primary" onclick="getOrderList()" style="width: 100%" value="조회">
                </div>
            <!-- </form> -->
        </div>
